...
class VehicleDetectionTracker:
    ...
    def process_video(self, video_path, result_callback, start_frame=0, end_frame=None):
        """
        Process a video by calling a callback for each frame's results from start_frame to end_frame.
        ...
        Args:
            ...
            end_frame (int): The frame number to stop processing at.
        """
        # Process a video frame by frame, calling a callback with the results.
        cap = cv2.VideoCapture(video_path)
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

        # Set the end frame to the total number of frames if not specified
